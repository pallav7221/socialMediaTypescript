...
const Posts: React.FC<PostData> = ({
  ...
  const addComment = (username: string, e: FormEvent) => {
    e.preventDefault();
  ...
  return (
    <div
      className="  max-w-xl mx-auto mb-8 p-6 border-gray-300 border rounded-3xl shadow-2xl transition duration-500 ease-in-out hover: bg - slate - 100 transformhover: -translate - y - 1 hover: scale - 105">
      ...
      <img className="rounded-3xl shadow-sm w-full" src={imgUrl} alt="" />
...
const Comment: React.FC<CommentProps> = ({
  ...
  const deleteCmnt = (e: React.MouseEvent<HTMLHeadingElement>) => {
    e.preventDefault();
  ...
  const editCmnt = (e: React.MouseEvent<HTMLHeadingElement>) => {
    e.preventDefault();
  ...
  const replyComment = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
          ...
          <div className="flex justify-between w-full gap-1">
            <div className="flex-col">
              ...
              <div className="mt-2 flex flex-row gap-5 text-blue-500 text-xs cursor-pointer ">
                {(comment?.data?.replies?.length ?? 0) >= 0 && (
                  <>
                    <h6
                      onClick={deleteCmnt}
